...
class UserAccountViewModel @Inject constructor(
    ...
    private val _editInfo = MutableStateFlow<Resource<User>>(Resource.Unspecified())
    ...
    fun updateUser(user: User, imageUri: Uri?) {
        ...
        if(areInputsValid) {
            viewModelScope.launch { _editInfo.emit(Resource.Loading()) }
            ...
            viewModelScope.launch { _editInfo.emit(Resource.Error("Check your inputs")) }
        }
    }
        ...
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media
                    .getBitmap(getApplication<Application>().contentResolver, imageUri)
                ...
                viewModelScope.launch { _editInfo.emit(Resource.Error(e.message.toString())) }
    ...
    private fun saveUserInformation(user: User, shouldRetrieveOldImage: Boolean) {
        firestore.runTransaction{transaction ->
            ...
            viewModelScope.launch { _editInfo.emit(Resource.Error(it.message.toString())) }
        }
